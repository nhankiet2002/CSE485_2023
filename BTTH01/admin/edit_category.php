<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM theloai WHERE ma_tloai = '$cat_id'";
        $result = mysqli_query($conn, $sql);

        // Kiểm tra xem thể loại có tồn tại hay không
        if ($result && mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $cat_name = $row['ten_tloai']; // Lấy tên thể loại
        } else {
        ...
        <div class="row">
            <div class="col-sm">
                ...
                <form action="process_add_category.php" method="post">
                    <!-- Mã thể loại, không cho sửa -->
                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text" id="lblCatId">Mã thể loại</span>
                        <input type="text" class="form-control" name="txtCatId" readonly value="<?php echo $cat_id; ?>">
                    </div>

                    <!-- Tên thể loại -->
                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text" id="lblCatName">Tên thể loại</span>
                        <input type="text" class="form-control" name="txtCatName" value="<?php echo $cat_name; ?>">
                    </div>
